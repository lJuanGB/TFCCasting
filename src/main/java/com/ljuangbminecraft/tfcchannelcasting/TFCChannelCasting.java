...
public class TFCChannelCasting {
    ...
    public void setup(FMLCommonSetupEvent event)
    {
        ...
        event.enqueueWork(TFCChannelCasting::registerInteractions);
        ...
        InteractionManager.register(
            ...
            (stack, context) -> {
                ...
                if (player != null)
                {
                    ...
                    level.getBlockEntity(posClicked, TFCCCBlockEntities.MOLD_TABLE.get()).ifPresent(
    ...
    public void registerPatchouliMultiBlock()
    {
        ...
        final IMultiblock multiblock = api.makeMultiblock(new String[][] {
            ...
            'R', api.stateMatcher(TFCBlocks.CRUCIBLE.get().defaultBlockState().setValue(PipeBlock.EAST, true)),
            'F', api.looseBlockMatcher(TFCBlocks.CHARCOAL_FORGE.get()),
            ' ', api.airMatcher(),
            'X', api.looseBlockMatcher(TFCBlocks.ROCK_BLOCKS.get(Rock.GRANITE).get(Rock.BlockType.BRICKS).get()),
            '0', api.looseBlockMatcher(TFCBlocks.ROCK_BLOCKS.get(Rock.GRANITE).get(Rock.BlockType.BRICKS).get()),
            '|', api.stateMatcher(TFCCCBlocks.CHANNEL.get().defaultBlockState().setValue(ChannelBlock.WEST, true).setValue(ChannelBlock.EAST, true)),
            '-', api.stateMatcher(TFCCCBlocks.CHANNEL.get().defaultBlockState().setValue(ChannelBlock.SOUTH, true).setValue(ChannelBlock.NORTH, true)),
            '+', api.stateMatcher(TFCCCBlocks.CHANNEL.get().defaultBlockState().setValue(ChannelBlock.SOUTH, true).setValue(ChannelBlock.NORTH, true).setValue(ChannelBlock.WEST, true)),
            'S', api.stateMatcher(TFCCCBlocks.CHANNEL.get().defaultBlockState().setValue(ChannelBlock.SOUTH, true)),
            'N', api.stateMatcher(TFCCCBlocks.CHANNEL.get().defaultBlockState().setValue(ChannelBlock.NORTH, true)),
            'C', api.stateMatcher(TFCCCBlocks.MOLD_TABLE.get().defaultBlockState().setValue(MoldBlock.SOUTH, true)),
            'c', api.stateMatcher(TFCCCBlocks.MOLD_TABLE.get().defaultBlockState().setValue(MoldBlock.NORTH, true))
        );
